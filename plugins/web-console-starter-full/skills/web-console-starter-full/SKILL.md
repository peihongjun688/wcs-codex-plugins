...
## Codex 加载（本 Skill 在 Codex 中的工作方式）

- 本技能安装为 `web-console-starter-full` 插件后，Codex 会把本文件所在目录作为技能根：`scripts/ docs/ backend/ frontend/ assets/ app.config.js` 均与 `SKILL.md` 同级。
- 触发场景：用户说"用 wcs 模板建系统 / 脚手架一套控制台 / 给控制台换肤 / 加皮肤 / 批量产出业务系统 / 做软著演示系统"。
- 核心动作：
  - 脚手架新系统：`powershell -ExecutionPolicy Bypass -File scripts/init-app.ps1 -AppName <name> -Title "<中文名>" -LogoText <X> -Skin <deep-blue/light-pro/emerald-dark/royal-purple> -Install`
  - 加皮肤：`scripts/new-skin.ps1`；校验五处名称一致：`scripts/check-names.ps1`；一键打包：`scripts/build-dist.ps1`
  - 产出目录即完整可运行工程（start.bat 一键起）。
- 桌面皮肤 4 套：deep-blue / light-pro / emerald-dark / royal-purple；H5 6 套（≤768px 自动切换）。详见本文件上文与 `docs/设计规范.md`、`docs/H5视觉规范v2.0.md`。
- 若用户想要**软著演示系统**：脚手架后真实运行一次（start.bat），确认登录/总览/列表/趋势/明细/设置全通，再考虑打包。
